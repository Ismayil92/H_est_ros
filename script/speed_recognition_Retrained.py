...
def checkStatus(croppedSymbol): #finding contour pixels on the particular region of the image
    global status
...
class image_converter: #To get Image as message
    ...
    def callback(self,data):
        try:
            ...
            #Speed recognition
            rh1 = [int(DigitCoordinates.speedupleft[0]), int(DigitCoordinates.speedupleft[1]), int(DigitCoordinates.speedbottomleft[1]), int(DigitCoordinates.speedupright[0])]
...
if __name__ == "__main__":
    ...
    #ROS PUBLISHER for showing speed data
    pub = rospy.Publisher('speed_info', screen, queue_size = 1)
    rospy.init_node('speed_info', anonymous = True)
    msg = screen()
    #rate = rospy.Rate(10)
    #For getting Digit and Symbol Coordinates
